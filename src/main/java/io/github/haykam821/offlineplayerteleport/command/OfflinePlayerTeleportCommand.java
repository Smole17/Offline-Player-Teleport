...
public final class OfflinePlayerTeleportCommand {
    ...
    private static int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        String suffix = offline ? "Teleported %s to offline player %s" : "Teleported %s to saved position of online player %s";
        context.getSource().sendFeedback(
                Text.literal(
                        "command.offlineplayerteleport.success" + suffix.formatted(
                                player.getName(),
                                target.getName())
                ), true);
